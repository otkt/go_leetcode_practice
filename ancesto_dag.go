...
func getAncestors(n int, edges [][]int) [][]int {
	...
	// map direct ancestors of nodes in a sorted maner
	for _, edge := range edges {
		sorted := directAncestor[edge[1]]
		appendSort(&sorted, edge[0])
		directAncestor[edge[1]] = sorted
	}
	res := [][]int{}
	godview := make(map[int][]int) //if a nodes ancestors are fully traversed hold them in map
	for i := 0; i < n; i++ {
		val := ta(i, &directAncestor, &godview) // traverse full ancesters recursive
		res = append(res, val)
	}
	return res
}
func appendSort(list *[]int, v int) {
	if len(*list) == 0 {
		*list = append(*list, v)
		return
	}
	var index int = len(*list)
	...
	*list = append((*list)[:index], append([]int{v}, (*list)[index:]...)...)
}

// traverse ancestors
func ta(node int, m *map[int][]int, g *map[int][]int) []int {
	// first check heap if node is already traversed before
	giga_sorted_arr, gok := (*g)[node]
	if gok {
		return giga_sorted_arr
	}
	//if not traverse recursive
	sorted_arr, ok := (*m)[node]
	if !ok {
		return []int{}
	}
	for _, a := range sorted_arr {
		if len(sorted_arr) == 0 {
			sorted_arr = ta(a, m, g) // this line probably not needed
		} else {
			sorted_arr = mergesorte(sorted_arr, ta(a, m, g))
		}
	}
	(*g)[node] = sorted_arr // write to heap to not traverse the same node again
	return sorted_arr
}

// merge already sorted arrays if value duplicate exists dont repeat it
func mergesorte(a_sorted []int, b_sorted []int) []int {
	if len(a_sorted) == 0 {
		return b_sorted
	}
	if len(b_sorted) == 0 {
	...
	ib := 0
	var mergesorted []int
	for ia < len(a_sorted) && ib < len(b_sorted) {
		if a_sorted[ia] == b_sorted[ib] {
			mergesorted = append(mergesorted, a_sorted[ia])
			ia++
			ib++
		} else if a_sorted[ia] < b_sorted[ib] {
			mergesorted = append(mergesorted, a_sorted[ia])
			ia++
		} else {
			mergesorted = append(mergesorted, b_sorted[ib])
			ib++
		}
	}
	for ia < len(a_sorted) {
